<template>
  <view>
    ...
    <view class="list-card" v-if="item.mode==='base'" @click="goArticleDetail">
      <view class="list-card-img" v-if='item.cover[0] || false'>
...
<script>
	export default {
		...
		methods:{
			goArticleDetail(){
				const {title,author,create_time,browse_count,thumbs_up_count,_id} = this.item;
				// author.avatar = author.avatar.replace(/&/g,'%26');
				// author.avatar = author.avatar.replace(/=/g,'%3D');
				const params = {title,author,create_time,browse_count,thumbs_up_count,_id}				
				// uni.navigateTo({
