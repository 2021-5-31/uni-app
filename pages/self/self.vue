<template>
	<view>
		<!-- 当用户为登录状态时进行显示 -->
		<view v-if="userInfo.userInfo" class="my-header">
			<view class="my-header-background">
				<image :src="userInfo.userInfo.avatar" mode="aspectFill"></image>
			</view>
			<view class="my-header-logo">
				<view class="my-header-logo-box">
					<image :src="userInfo.userInfo.avatar" mode="aspectFill" @click="changeAvatar"></image>
				</view>
				<text class="user-name">{{userInfo.userInfo.author_name}}</text>
			</view>
			<view class="my-header-info">
				<view class="my-header-info-box">
					<text class="my-header-info-title">被关注</text>
					<text>{{userInfo.userInfo.follow_count}}</text>
				</view>
				<view class="my-header-info-box">
					<text class="my-header-info-title">粉丝</text>
					<text>{{userInfo.userInfo.fans_count}}</text>
				</view>
				<view class="my-header-info-box">
					<text class="my-header-info-title">积分</text>
					<text>{{userInfo.userInfo.integral_count || 0}}</text>
				</view>
			</view>
		</view>

		<view class="my-content">
			<view v-if="!userInfo.userInfo" class="my-content-list" @click="goLoginPage">
			...
			<!-- #endif -->
			<button v-if="userInfo.userInfo" type="warn" class="sign-out" @click="siginOut">退出</button>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 退出登录
			siginOut() {
				this.$store.commit('userInfo/setUserInfo', null)
				uni.navigateTo({
					url: '/pages/self/self'
				})
			},
			...
			async _checkVersion() {
				const {
					version,
					downLoadLinkUrl
				} = await this.$api.getCurrentVersion();
			...
			async changeAvatar() {
				const imgInfo = await uni.chooseImage({
					count: 1,
				})
				const imgFile = {
					filePath:imgInfo[1].tempFilePaths[0],
					cloudPath:imgInfo[1].tempFiles[0].name
				}
				const {fileID} = await uniCloud.uploadFile(imgFile);
				const res = await this.$api.changeUserAvatar({
					userId:this.userInfo.userInfo._id,
					filePath:fileID
				})
				this.$store.commit('userInfo/setUserInfo',{...this.userInfo.userInfo,avatar:fileID})
				uni.showToast({
					title:res.msg,
					icon:'none'
				})
